...
const testPage = async function () {
	...
	await synthetics.executeStep('STEP 2 - Check CMP', async function () {
		...
		await page.screenshot({ path: `/tmp/step2-cmp-${pageType}.png` });
		log('Page loaded');
	...
	await synthetics.executeStep(
		...
		async function () {
			...
			await reloadPage(page);
			await page.screenshot({ path: `/tmp/step4-cmp-${pageType}.png` });
	...
	await synthetics.executeStep(
		...
		async function () {
			...
			await page.screenshot({ path: `/tmp/step9-cmp-${pageType}.png` });
	...
	await synthetics.executeStep('STEP 10 - Pageskin', async function () {
		...
		await page.screenshot({ path: `/tmp/step10-pageskin-${pageType}.png` });
		log('Pageskin loaded');
	});
};
